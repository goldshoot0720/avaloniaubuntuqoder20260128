...
public partial class MainWindow : Window
{
    ...
    private Process? _mediaPlayerProcess;
    ...
    private void ShowVideoIntroView()
    {
        ...
        playButton.Click += async (s, e) => await PlayVideoAsync();
        ...
        stopButton.Click += (s, e) => StopMedia();
        featuredStack.Children.Add(stopButton);
    ...
    private void ShowMusicLyricsView()
    {
        ...
        playButton.Click += async (s, e) => await PlayMusicAsync();
        featuredStack.Children.Add(playButton);
    ...
    private async Task PlayMusicAsync()
    {
        try
        {
            ...
            var players = new[] 
            { 
                new { Name = "mpg123", Args = "" },
                new { Name = "vlc", Args = "--play-and-exit" },
                new { Name = "cvlc", Args = "--play-and-exit" },
                new { Name = "ffplay", Args = "-nodisp -autoexit" },
                new { Name = "mplayer", Args = "" }
            };
            ...
            foreach (var player in players)
            {
                try
                {
                    var psi = new ProcessStartInfo
                    {
                        ...
                        Arguments = $"{player.Args} \"{_musicUrl}\"",
                    ...
                    _mediaPlayerProcess = Process.Start(psi);
                    if (_mediaPlayerProcess != null)
                    {
                        Debug.WriteLine($"使用 {player.Name} 播放音樂");
                        ShowNotification($"正在使用 {player.Name} 播放音樂...");
                        return;
                    }
                }
                catch
            ...
            // 如果沒有找到播放器，使用預設瀏覽器
            Debug.WriteLine("未找到音樂播放器，使用瀏覽器開啟");
            ShowNotification("未找到系統播放器，將在瀏覽器中開啟...");
        ...
        {
            Debug.WriteLine($"播放音樂失敗：{ex.Message}");
            ShowNotification($"播放失敗：{ex.Message}");
        }
    }
    ...
    private async Task PlayVideoAsync()
    {
        try
        {
            ...
            foreach (var player in players)
            {
                try
                {
                    ...
                    {
                        FileName = player.Name,
                        Arguments = $"{player.Args} \"{_videoUrl}\"",
                        UseShellExecute = false,
                        RedirectStandardOutput = true,
                        RedirectStandardError = true,
                    ...
                    if (_mediaPlayerProcess != null)
                    {
                        Debug.WriteLine($"使用 {player.Name} 播放影片");
                        ShowNotification($"正在使用 {player.Name} 播放影片...");
                        return;
                    }
                }
                catch
            ...
            // 如果沒有找到播放器，使用預設瀏覽器
            Debug.WriteLine("未找到影片播放器，使用瀏覽器開啟");
            ShowNotification("未找到系統播放器，將在瀏覽器中開啟...");
        ...
        {
            Debug.WriteLine($"播放影片失敗：{ex.Message}");
            ShowNotification($"播放失敗：{ex.Message}");
        }
    }
    ...
    private void StopMedia()
    {
        try
        {
            if (_mediaPlayerProcess != null && !_mediaPlayerProcess.HasExited)
            {
                _mediaPlayerProcess.Kill();
                _mediaPlayerProcess.Dispose();
                _mediaPlayerProcess = null;
                Debug.WriteLine("已停止媒體播放");
                ShowNotification("已停止播放");
            }
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"停止媒體失敗：{ex.Message}");
        }
    }
    ...
    private void ShowNotification(string message)
    {
        ...
        Avalonia.Threading.Dispatcher.UIThread.Post(() =>
        {
            ...
            {
                var contentControl = this.FindControl<ContentControl>("MainContentControl");
                if (contentControl?.Content is ScrollViewer scrollViewer && 
                    scrollViewer.Content is StackPanel panel)
                {
                    ...
                    // 如果已經有通知，移除舊的
                    if (panel.Children.Count > 0 && panel.Children[0] is Border firstBorder)
                    {
                        if (firstBorder.Background is SolidColorBrush brush && 
                            brush.Color == Color.Parse("#3498DB"))
                        {
                            panel.Children.RemoveAt(0);
                        }
                    }
                    
                    panel.Children.Insert(0, notification);
                    ...
                    Task.Delay(3000).ContinueWith(_ =>
                    {
                        Avalonia.Threading.Dispatcher.UIThread.Post(() =>
                        {
                            if (panel.Children.Contains(notification))
                            {
            ...
            {
                Debug.WriteLine($"顯示通知失敗：{ex.Message}");
            }
        });
    }
}